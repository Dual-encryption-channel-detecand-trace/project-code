...
function data2circle(data)
{
    ...
    for(let i=0;i<data.length;i++)
    {
        if(data[i].value/sum>leastsize)
        {
            showdata.push(
                {   value: data[i].value
                ,   name: data[i].name
                });
            if(data[i].name=="tormeek")
            {
...
function data2line(data)
{
    ...
    for(let t=0;t<data.length;t++)
    {
        for(let i=0;i<data[t].length;i++)
        {
            ...
            if(found==false)
            {
                showo.push([(t+1)*eps,data[t][i].value]);
                showdata.push(
                    {   name: data[t][i].name
                    ,   type: "line"
...
// 作图
//扇形图
function circlegraph(data,containerid)
{
    data=data2circle(data);
    // 作图
...
//折线图
function linegraph(data,containerid)
{
    data=data2line(data);
